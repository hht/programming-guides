...
> **状态**：候选规格（对抗 **5/5 PASS** 前勿标 DONE；通过后删本句「候选」字样）。 
> **问题**：编程语言规范应如何落入本仓。 
> **相关**：[AGENTS.md](../../AGENTS.md) · [how-to-write-guides.md](./how-to-write-guides.md) §1.2 · [naming-business-first.md](./naming-business-first.md) · [adversarial-review.md](./adversarial-review.md)。
...
| # | 规约 | 探针 / 判据 |
|---|------|-------------|
| D1 | **无用优先删除**：不可达、无引用、仅「以后可能用」的代码/文件/依赖/配置 → **先删**；恢复靠 git，不靠注释掉大段 | PR/`check`：死代码门闸（语言对应 linter unused / 等价）或评审列出「保留理由」一行；无理由 = FAIL |
| D2 | **能删层则不封装**：若去掉某一 wrapper / Helper / Manager / 中间 interface，调用方仍能完成**同一用户可观察行为** → **禁止**该层；直接调下一层 | 设计/实现评审：画出「删掉该类型后是否仍一行调用可达」；仍可达却保留 = FAIL |
| D3 | **封装准入**：仅当出现 **≥2** 处稳定重复 **且** 有独立不变式/失败分类要单测时，才允许抽函数/类型；禁止「先封装再等调用方」 | 新类型 PR 须写：重复次数或不变式一句；否则删 |
| D4 | **指南同构**：`language-gates` 与应用册规格禁为「完整性」堆未使用章节/门闸；能合并进已有节则合并 | 对抗：无探针的门闸、无挂靠册的 gate 段落 → 删 |
...
为每门默认栈语言落一份**薄闸**：用最少机械规则，强制产出**极简、可测、算法得当**的代码；formatter/linter 只做仆人，不写语言百科，不抢 Pass1 业务词表。
...
1. [ ] §3.3 各节齐全（含**最高准则映射**表） 
2. [ ] fmt+lint 无「或」开口 
3. [ ] 硬门闸 ∈[1,30]；每条有探针 + 归属 §0 
...
落盘路径不清=FAIL；缺某语言具体例句≠FAIL。
```

| 角 | 深挖 |
|----|------|
| A | Lifecycle 1–7 + §9 |
| B | Lifecycle + 失败分类；§0 映射可执行 |
| C | 路径与 SSOT（含 §0 凌驾 Google） |
| D | `language-gates/` 树 |
| E | 齐套、11 固定句、Focused vs Full |
