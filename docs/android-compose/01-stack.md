...
**Kotlin** + **Compose BOM** + **ViewModel** + **UiState `data class`**（`StateFlow`）+ **Navigation Compose** + **Coroutines + Flow**；测试 **Turbine** + **Compose UI Test**；DI **默认构造注入/简单工厂**（Hilt 可选且须 INPUTS 书面）。
...
| 构建 | Gradle Kotlin DSL + Version Catalog（推荐） | 锁版本进 lock / catalog；指南不钉 semver 数字 |
| 网络（若有） | **OkHttp + Retrofit**（默认）；Ktor Client 仅 INPUTS 书面迁移理由 | 禁在 Composable 里直接 `HttpURLConnection` |
...
锁版本：实现仓 Version Catalog / lockfile；本指南钉**能力边界**，不钉具体版本号。
