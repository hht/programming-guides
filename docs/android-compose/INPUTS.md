...
| # | 项 | 验收 |
|---|-----|------|
| 1 | **应用标识** | `applicationId`（如 `com.example.app`）+ 展示名；minSdk **默认 26**（改则写明数字 + 理由） |
| 2 | **设计稿** | frame/screen 清单：至少 1 个主任务流；每屏含 **loading / empty / error / success**（或标明 N/A 谓词） |
...
| 5 | **鉴权（若有）** | □ 无 □ 对接 [docs/auth](../auth/README.md) 模式（写模式字母）□ 其它（须写明 token 存放：默认 **EncryptedSharedPreferences / 系统凭证**；**禁**明文 SharedPreferences 存 session） |
...
| 10 | **DI** | □ **默认**：构造注入 / 简单工厂（无 Hilt）□ **Hilt**（须写明：模块边界 + 测试替身策略） |
| 11 | **测试设备** | 至少：1 个 emulator API 级 + 1 个物理或第二 API；Compose UI Test 跑在哪须写明 |
