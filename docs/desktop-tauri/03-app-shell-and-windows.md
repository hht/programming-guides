...
1. **配置壳**  
   ...
   - 单窗默认 label **`main`**；尺寸/最小尺寸书面或 conf 钉死。

2. **前端入口**  
   - Vite + React（或 INPUTS 钉的册）挂载；开发态 `tauri dev` 热更新。  
...
3. **多窗（若 INPUTS §12）**  
   ...
   - 共享状态：默认 **Rust 侧 App 状态 + 事件/查询**；禁多窗各持一份冲突的「权威会话」除非 INPUTS 钉明。
...
| 多窗同实体编辑冲突 | INPUTS 钉：后写覆盖 / 锁 / 只读副本 — 默认**单写者** |
