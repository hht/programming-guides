...
- **顺序固定**：先 [docs/auth](../auth/README.md) Session Gate（得 Subject）→ 再本 Gate；无 Subject → `UNAUTHENTICATED`（不进入租户步骤）。 
...
| 5 | **Audit** | 对 INPUTS 要求的动作（默认：成员/角色/权限变更、计费状态变更；可选：敏感读）追加 **AuditEvent**（who=subject、tenant_id、action、resource、at）。审计写失败策略：INPUTS 约定「阻断请求」或「记错误指标仍放行」；**默认：变更类阻断**。 |
...
| `BILLING_INACTIVE` | 计费状态不允许该写 | 402 或 403（INPUTS 择一） | 无计费则 N/A |
