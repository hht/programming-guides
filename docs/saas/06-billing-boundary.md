...
- **不**约定唯一支付商；INPUTS 勾选 Stripe 或其它。 
- 席位：邀请/加成员前检查上限；超额按 INPUTS §17（拒绝或只读降级）择一。
...
3. **写门闸**：Tenant Gate 步骤 3：若 `isWrite` 且 status ∈ 阻断集（默认 `past_due` 超宽限期、`canceled`）→ `BILLING_INACTIVE`。读是否允许：INPUTS 约定（默认读允许）。 
...
| 席位已满邀请 | 409 或 403（INPUTS 约定）；不创建 active 成员 |
