...
| `billing-inactive` | 计费停用阻断写（若启用） | 提示升级/续费；读可按产品钉 |
...
| Term | 含义 | 代码符号 | 禁同义词 |
|------|------|----------|----------|
| Tenant | 租户边界实体 | `Tenant` | Org/Workspace/Account **混用**（择一钉死为 Tenant 或在词表声明别名唯一映射） |
...
| 头 / 路径 | `X-Tenant-Id`（或 INPUTS 钉死名）；slug kebab |
