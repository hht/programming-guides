...
2. **供应商互斥**：INPUTS **恰好选定一家**；适配器可换，**投递状态机与幂等语义不随供应商分叉**。 
...
6. **Webhook 必须验签**（或等价 mTLS / SNS 订阅确认策略，INPUTS 写明）；验签失败 → `EMAIL_WEBHOOK_INVALID`，不推进状态。 
7. **hard bounce / complaint → 抑制表**；被抑制地址默认拒绝新发送（`EMAIL_SUPPRESSED`），除非 INPUTS 写明覆盖流程。 
