...
- **发送路径互斥**：INPUTS §6 采用 **异步队列（推荐）** 或 **同步直发**。 
...
1. 在业务事务内插入 `email_messages`：`state = queued`（或 `composed`→立刻 `queued`），payload = composed 快照或「template_id+variables」二选一（**默认存 composed 快照**，避免模板升版竞态；INPUTS 可改存引用并约定风险）。 
