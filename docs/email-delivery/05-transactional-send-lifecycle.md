...
- **provider ack ≠ delivered**：`submitted`/`accepted` 后仍须步骤 4 事件或超时策略（INPUTS 可钉「仅追踪至 accepted」书面裁剪，默认追踪至 delivered/bounced/complained）。
...
1. Webhook（或 SNS/轮询，INPUTS 钉）收事件 → **验签**。  
