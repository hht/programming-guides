...
| # | 项 | 验收 |
|---|-----|------|
| 1 | **供应商（互斥钉死恰好一家）** | □ **Resend** □ **Postmark** □ **Amazon SES** □ **其它**（须书面理由 + 官方 API/Webhook 文档 URL）— **禁止**「Resend 或 SES 任选」双开口；指南不钉百科，**本项钉死运行时一家** |
...
| 5 | **幂等键策略** | 每封出站意图 **必填** `idempotency_key`（业务词根+意图维度，见 `07`）；唯一性范围钉死：□ 全局 □ 按 `template_id`；冲突行为钉死：□ **reject**（默认） □ coalesce（书面） |
| 6 | **发送路径（互斥钉死一）** | □ **同步直发**（请求内 compose→provider；仅低流量/可丢延迟预算书面） □ **异步队列**（**推荐**；对齐 [workers-queue](../workers-queue/README.md)，队列名例 `email.send`）— **禁止**未钉死双开口 |
| 7 | **Webhook / 事件入口** | 公开 HTTPS 路径（或内部队列转发）+ **签名校验**策略（供应商文档方法名/头名）；staging/prod endpoint **成对** |
| 8 | **投递状态机** | 确认采用本册 `06` 状态枚举（或书面差分表：旧名→本册名）；**禁止**仅以供应商仪表为唯一真相而无应用侧记录 |
| 9 | **Bounce / complaint 策略** | hard bounce → 抑制；complaint → 抑制；soft bounce → 重试上限数字（默认对齐 workers-queue 或钉 **3**）；抑制保留默认 **90 天**或书面 |
...
| 14 | **出站与业务同事务** | 异步默认：业务写与 **email outbox / enqueue** 同事务（对齐 workers-queue `04`）；若拆事务须书面 + 失败补偿 |
| 15 | **附件** | □ 不做（默认）□ 做：最大字节、MIME 白名单、禁可执行类型 |
| 16 | **批量 / 营销信** | □ 不做（默认；本册主路径=事务）□ 做：须书面与事务队列隔离（独立 `template_id` 前缀或队列名）+ 退订合规一句 |
| 17 | **供应商 SDK** | □ 官方 HTTP 客户端自封装（默认可）□ 官方 SDK（须版本钉死）；**禁止**同时两套适配器作 SSOT |
