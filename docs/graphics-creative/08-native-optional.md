...
- **系统减动效**：默认尊重（iOS `UIAccessibility.isReduceMotionEnabled` / Android `Settings.Global.ANIMATOR_DURATION_SCALE` 或等价）；关关键持续动画或时长→0。**仅**当 INPUTS §13 书面「不降级」时矩阵行 5 写 N/A（仍须帧预算测量）；不得以「未接系统 API」跳过行 5——须接入上述 API（或平台等价）或走 §13 书面。
