...
> **默认栈（Web）**：**CSS `transform` / `opacity`（及白名单内 `filter` / `clip-path`）** + **`requestAnimationFrame` / `IntersectionObserver`**；**禁止**把 Framer Motion / GSAP / Lottie 钉为默认依赖。原生可选章：SwiftUI / Compose 动画对齐同一帧预算原则。  
