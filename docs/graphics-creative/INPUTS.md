...
| 2 | **目标 fps（互斥主档）** | □ **60**（默认；帧预算 **16.67 ms**） □ **120**（须写明设备档；帧预算 **8.33 ms**） □ **30**（仅允许作**减配档**，不得作未减配前的主档） |
...
| 8 | **禁默认库确认** | Web：确认**不**把 Framer Motion / GSAP / Lottie 作默认依赖；若例外引入须在 INPUTS 写明理由 + 仍遵守白名单与预算 |
| 9 | **列表/长滚动（若有）** | □ 无长列表 □ 有：须说明虚拟化/回收策略（学 flash-list 边界，不把它当作唯一 RN） |
...
| # | 项 |
|---|-----|
| 13 | `prefers-reduced-motion`：默认 **尊重**（减配到瞬时或关）；产品明示「不降级」须写明，并附 仍保留帧预算测量 |
