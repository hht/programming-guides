...
| 网络 | `URLSession` + 自管 API client（或 INPUTS 约定的薄客户端） | 禁在 View body 直接 `URLSession` 无 Store |
| 持久化 | 默认按 INPUTS：轻量 `AppStorage`/`File`；结构化 → **SwiftData** 或写明 SQLite | 禁第三套平行「随便 UserDefaults 塞大 JSON」无边界 |
...
xcodebuild -version # 确认 Xcode；本指南不指定具体 Xcode 大版本号，约定能力：Swift 6 + SwiftUI Observation
```

锁版本：用 Xcode 工程 / `Package.resolved` 锁定依赖；本指南不指定具体 semver 数字，约定**能力边界**（Swift 6、Observation、NavigationStack、Swift Testing）。
