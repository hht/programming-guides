...
| 2 | **最低部署版本** | 钉死 iOS / iPadOS / macOS 最低版本（须支持 SwiftUI `NavigationStack` + Observation；建议 iOS 17+ / macOS 14+；更低须书面桥接成本） |
...
| 5 | **API / 本地数据契约** | □ 远程 API：OpenAPI/字段表 + 错误码 □ 纯本地：模型字段表 + 持久化边界（UserDefaults/SwiftData/文件）须钉死；二者可并存 |
| 6 | **鉴权（若适用）** | □ 无 □ 有 — 对接 [docs/auth](../auth/README.md) 或书面等价；token 落点禁 Keychain 以外明文 |
...
| 8 | **Bundle ID / 签名** | 开发 Team、Bundle ID 模式（staging/prod 后缀策略书面）；CI 用何证书策略一句 |
...
| 10b | **同 Route 重复 push** | 互斥钉死一种：□ **允许**栈内重复同 Route □ **去重**（已在栈则 pop 到该 Route 或忽略二次 push——书面择一谓词）。默认未勾 → **允许** |
| 11 | **Mac 面（§1 含 Mac 时必填）** | 主窗口策略（单窗 / 多窗）；菜单栏命令表（至少 Quit / 偏好 / 主操作）；键盘快捷键表。§1 无 Mac → 本行 `N/A` |
| 12 | **无障碍与动态字体** | □ 遵守 HIG：VoiceOver 标签计划 + Dynamic Type；裁剪须书面一行理由 |
