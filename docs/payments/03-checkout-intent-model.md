...
3. **Checkout 入口**：校验 Subject（对齐 auth Session Gate）；若 saas 启用则校验租户成员与 `billing:manage` 或购买权限（INPUTS 钉权限码）。  
...
| 幂等键冲突（默认 reject） | `PAYMENT_ALREADY_SETTLED` 或专用 `IDEMPOTENCY_CONFLICT`（INPUTS 钉）；不新建 |
