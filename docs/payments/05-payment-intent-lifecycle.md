...
- 顺序固定：**create intent → confirm → webhook verify → settle / fail / refund boundary**。 
- **终态权威 = 验签后的 webhook（或 INPUTS 写明允许的服务端权威 retrieve）**；客户端 confirm / success URL **不得**单独 settle。 
...
4. 提供商失败 → 不留下「已付」；返回 `PROVIDER_UNAVAILABLE` 或删除/标记失败创建（INPUTS 约定：默认保留行 + `failed` 仅当提供商明确拒绝）。
...
3. 退款成功映射：若 saas 启用，按 INPUTS 决定是否 `canceled` / `past_due` / 降级——**不得**静默保持 `active` 却已全额退（须写明例外）。
