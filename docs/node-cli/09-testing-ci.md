...
**跑法：** `pnpm test:e2e` 用 Vitest；每个 case `spawn('node', ['build/cli.js', ...args], { env })`（或 bin 路径），断言 `exitCode` + stdout/stderr 子串。 
**前置：** `pnpm build`。 
...
| 2 | 默认命令 **CI 成功入口**（INPUTS §9 所约定 argv；无 TTY） | exit 0；若有机器 schema 则 stdout 可解析 |
| 3 | 非法 flag | exit 2；stderr 含 `ERROR:USAGE` |
| 4 | 交互命令**不带** CI 成功 flag，且非 TTY | exit **2**；stderr `ERROR:USAGE` |
| 5 | 取消（可测则测） | exit 130 |

说明：#2 与 #4 不冲突——#2 走 INPUTS 约定的无 TTY 成功 argv；#4 走需要 TUI 的调用。真实 PTY：**宜做**。
