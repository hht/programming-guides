...
| 3 | **设计稿** | frame/screen 清单：至少 1 个主任务流；每屏含 **loading / empty / error / success**（或书面 N/A 谓词） |
...
| 6 | **鉴权（若有）** | □ 无 □ 对接 [docs/auth](../auth/README.md) 模式（写模式字母）□ 其它（须钉 token 存放：默认 **SecureStore**；**禁** AsyncStorage 明文存 session） |
...
| 9 | **失焦取消策略** | 每屏勾选：□ focus 加载须 AbortController/代次取消 □ 无 inflight（静态屏，书面 N/A）— 与 `07` 对齐 |
...
| 12 | **工作流** | □ **默认 managed**（EAS Build；禁裸 `eject` / 手工维护 `android/`+`ios/` 作默认）□ **CNG / prebuild 定制**（须书面：为何 + 仍走 `expo prebuild`，非永久 eject） |
| 13 | **测试设备** | 至少：1 个 iOS Simulator + 1 个 Android Emulator（或 INPUTS 写死的托管设备）；RNTL / Maestro 跑在哪写死 |
