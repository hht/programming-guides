...
|---|-----|------|
| 1 | **用户任务** | 角色+成功标准 |
| 2 | **页面/路由清单** | `path` \| RSC/CSR \| 主 CTA \| **路由四态**（列名钉死：`default`/`loading`/`empty`/`error`，与 [templates/page-state-matrix.md](./templates/page-state-matrix.md) 一致）\| **公开?** □是□否 |
...
| 5e | **登录表单字段**（仅 §5=Cookie） | 默认 `email` + `password`（字段名钉死）；改字段须书面列表。签发入参=校验通过后的 `sub` |
