...
- Streams 默认：**唯一**路径 = **outbox + 转发 `XADD`**（INPUTS §17）；其它路径仅 §17 书面。  
- 禁止「先返回成功再尽力入队」而无 outbox/补偿书面方案。
...
3. **持久化**  
   ...
   - **Streams**：**默认** 业务与 **outbox 同行同事务**写入，转发器再 `XADD`（INPUTS §17）；**禁止**规格默认「业务后直接 XADD / 告警+重试入队」与 outbox 并列开口。其它路径仅 §17 书面。**禁止**入队成功但业务未提交却已执行。  

4. **返回**  
   - `job_id`（PG uuid/bigint 或 Stream entry id）给调用方；不暴露内部 claim token。  
5. **延时入队（可选）**  
   - `visible_at = now() + delay`；Streams 用延迟队列或 `visible_at` 旁路表——须 INPUTS 书面；默认可不支持。
