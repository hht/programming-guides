...
> **默认栈**：**PostgreSQL `SELECT … FOR UPDATE SKIP LOCKED` 队列表**（应用已有 PG 权威源时**钉死**；对齐 [postgres](../postgres/README.md)）或 **Redis Streams**（无 PG / INPUTS 显式选流时）；**BullMQ / Sidekiq 仅作映射学习，不钉默认**。幂等键**必填**；可见性超时与最大重试钉死或进 INPUTS。**禁止** `setTimeout` / 进程内数组冒充队列。  
