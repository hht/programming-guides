...
4. **心跳 / 续期（可选）**  
   - 长任务：周期性把 `visible_at` 后推（或 `XCLAIM` 续期）；**默认不续期**——要求 handler 短于超时，或 INPUTS 勾选续期并钉周期。  
5. **丢失认领**  
   - Worker 崩溃：不 ack；到期后其他 Worker claim；`attempt` 再 +1（注意与「执行失败 retry」共用上限 —— 崩溃重领也计 attempt，**默认计**；若产品要区分须 INPUTS 书面两计数器）。
...
| claimed_by 不匹配仍 ack | 拒绝或幂等 no-op（钉一种；默认 **拒绝** → `JOB_NOT_CLAIMED`） |
