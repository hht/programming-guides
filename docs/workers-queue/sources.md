...
|----|------|------|--------|----------|--------------|
| A | [riverqueue/river](https://github.com/riverqueue/river) | P1 | PG 队列表 + `SKIP LOCKED` 认领、重试/死信形状 | 照搬 Go API / River 专有仪表 | 用 Postgres 做可靠后台 Job |
| B | [sidekiq/sidekiq](https://github.com/sidekiq/sidekiq) | P1 | 入队→Worker→重试→死信用户流；幂等/唯一 Job 习惯 | 将 Redis+Ruby 定为指南默认引擎；抄 Sidekiq Pro | 应用把工作丢进队列由 Worker 执行 |
| C | [taskforcesh/bullmq](https://github.com/taskforcesh/bullmq) | P1 | 锁时长/可见性、attempts、失败进 failed 集 | 将 BullMQ/Redis 定为有 PG 时的默认引擎 | Node 应用异步 Job 与重试 |

映射学习（非 B 共有证据源、不作默认）：Faktory 文档、Cloudflare Queues 文档 — 仅当需对照托管语义时引用。
...
| 可见性超时 + max attempts 写明 | A,C | 工程 | `06`/`08`/INPUTS | 必做 |
...
| 冲突 | 裁决 |
|------|------|
| Sidekiq / BullMQ 下载与生态更大 | **不作默认引擎**；作 **映射学习**（重试/死信/锁时长语义对照）。有 PG 权威源 → **采用 PG SKIP LOCKED** |
