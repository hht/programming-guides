...
| **本地开发** | **MinIO**（Docker Compose；path-style 或 virtual-host 按 INPUTS 选定一种） |
...
实现仓按应用主语言**选定一行**；跨语言服务各进程各自选定，禁止同进程双客户端库。
...
| 其它 | INPUTS 写明：官方 S3 兼容 SDK 名 + 预签名 API | 同左 |

若 INPUTS 写明改选同语言另一主流客户端（例 TS 改 minio-js），须：① 冲突表理由；② 仍映射到本册 Lifecycle；③ **不得**双 SSOT。
...
| minio-go | 大版本 **v7** |
| boto3 | 跟应用 Python 锁文件；S3 API 调用面本册内约定 |
