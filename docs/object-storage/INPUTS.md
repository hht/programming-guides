...
| # | 项 | 验收 |
|---|-----|------|
| 1 | **应用语言（钉死一）** | □ TypeScript/JavaScript □ Go □ Python □ 其它（书面 SDK 名）— 决定 `01` 客户端行 |
| 2 | **S3 兼容 endpoint** | staging/prod **成对** `S3_ENDPOINT`（本地可 `http://127.0.0.1:9000`）；**值策略书面**；密钥**不入库** |
| 3 | **凭证** | 成对 `S3_ACCESS_KEY_ID` / `S3_SECRET_ACCESS_KEY`（或实现仓等价名）；仅服务端 |
| 4 | **桶名** | `S3_BUCKET` 钉死；默认 **private**；禁止默认 public-read |
| 5 | **区域** | `S3_REGION`（MinIO 常用 `us-east-1` 占位）；钉死字符串 |
| 6 | **寻址风格** | □ path-style（**本地 MinIO 默认**）□ virtual-hosted — 互斥；对应 `S3_FORCE_PATH_STYLE` |
| 7 | **Put 模式（主路径）** | □ **presign put**（浏览器默认）□ **server put**（Worker 默认）— 主 call site 钉死一种 |
| 8 | **预签名 TTL** | Put 默认 **300s**；Get 默认 **60s**；改则写入秒数 |
| 9 | **大小上限** | `max_bytes` 默认 **20 MiB**（20971520）；改则钉字节数 |
...
| 17 | **verify 残留** | **默认删除**；改隔离前缀须钉前缀规则 |
| 18 | **Multipart** | 仅当单对象常 > **100 MiB**；钉阈值与 part size |
