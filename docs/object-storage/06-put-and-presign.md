...
| **server put**（默认·Worker） | 可信后端、小文件、需同步扫描 | 浏览器经应用无限制中转超大 body（须写明 max） |

同一产品可两种都实现，但**每个 call site** 选定一种；禁止「有时随便」。
...
| 强制 path-style 与 endpoint 不匹配 | 配置错误；本地 MinIO 常见，INPUTS 约定 `S3_FORCE_PATH_STYLE` |
