...
3. 模式（INPUTS 约定 call site）： 
 - **presign get**（默认）：TTL = `PRESIGN_GET_TTL_SECONDS`（**默认 60**） 
 - **server get**：服务端流式转发（小文件、需审计包一层时） 
4. 不存在 → `object.not_found`（与 forbidden 分码，避免存在性泄漏时：INPUTS 约定「统一 not_found」或「真 forbidden」——**默认对无权限统一 `object.forbidden`**，对有权限但缺失用 `not_found`）。
