...
| C | [nomisRev/ktor-arrow-example](https://github.com/nomisRev/ktor-arrow-example) | Ktor RealWorld 工程示例（kotlinx.serialization） | 错误处理、模块边界、可测布局、配置分离 | 绑死 Arrow / SqlDelight 为默认（本指南钉 Exposed + `AppError`） |
...
> **共有** = 用户可感知能力在 \(B\) 的 ≥2 证据源出现（与 [../go/](../go/sources.md) / [../fastapi/](../fastapi/sources.md) 同构切条；上表与 `11` §B 同构）。**可测布局**（`Application.module` / test host）与 **`/healthz`+`/readyz` 探针**属工程面（后者为指南硬钉，非共有用户可感知切条）→ 见差距表；实现仍见 `01`/`03`/`09`。
...
| `/healthz`+`/readyz` 探针 | 工程（指南硬钉） | 工程 | `03`/`09` | 必做（工程；**非共有**） |
...
| 冲突 | 裁决 |
|------|------|
| Spring Boot 更「企业流行」 | **钉 Ktor**（协程一等、轻、插件边界清晰；先进优先；Spring 不进默认；若产品强制 Spring → 另册） |
| Hibernate/JPA 更流行 | **钉 Exposed**（Kotlin DSL + 显式事务；与 Ktor suspend 同构） |
| jdbi ≈ sqlc、SQL-first 更「纯」 | **钉 Exposed**（本册先进默认；jdbi 不进默认，勿双栈） |
| Jackson 仍为 JVM JSON 默认 | **钉 kotlinx.serialization**（与 Kotlin 类型系统同构） |
| Arrow / 函数式错误轨道（C） | **不钉 Arrow**（C 学边界；默认 `AppError` + StatusPages） |
| SqlDelight（C）vs Exposed | **钉 Exposed**（与 B/A 可映射；禁默装第二套 ORM） |
