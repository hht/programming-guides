...
| 连接建立 / idle（Netty） | idle **60s**（与引擎配置对齐，单处钉） |
...
2. 插件顺序（钉死）：`CallId`/`RequestId`（见 `05`）→ **StatusPages**（见 `04`）→ ContentNegotiation(json) → CallLogging（带 `request_id`）→ **Auth（仅受保护 route）** → Routing。  
