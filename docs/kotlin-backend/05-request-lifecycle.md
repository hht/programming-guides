...
2. **插件 / 路由顺序**：RequestID/`CallId` → **StatusPages**（见 `04`；未捕获 → `INTERNAL` JSON，**禁止**默认明文 500）→ ContentNegotiation → CallLogging（带 `request_id`）→ **Auth（仅受保护 route）** → handler。Auth **在**业务用例之前。 
3. **Handler 内顺序**： 
