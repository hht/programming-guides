...
| 3 | **校验有效性** | **A/B/C**：`hash(token)` 查库；未撤销；`expires_at`/`idle_expires_at`。**D**：验 JWT 签名/iss/aud/exp（`06`）；revocation 若启用则查黑名单。存储/JWKS 错误 → **STORE_UNAVAILABLE** 或按未认证（INPUTS 钉）。无效 → **SESSION_INVALID** / **SESSION_EXPIRED**（JWT 过期映射为 SESSION_EXPIRED）。 |
