...
| 3 | **Cookie 名** | 默认 **`session`**；改则写死（全栈唯一）。模式 D → `N/A` |
...
| 13 | **CSRF** | 模式 A/B/C 写操作：默认双重提交（见 `07`）或书面 SameSite 严格策略。模式 D → `N/A` |
| 14 | **应用册对接** | □ nextjs □ react □ go □ fastapi □ 多册（列清单）— 本册为鉴权 SSOT（`08`） |
| 15 | **IdP** | □ 无（自管） □ Ory Kratos（可选） □ 其它（须钉 issuer+JWKS URL；**禁止** Keycloak 唯一默认） |
