...
| 5 | **鉴权策略** | 勾选其一：□ 无登录  □ Cookie session（`credentials:include`；SameSite；若需 CSRF 写头字段名） □ Bearer **仅 memory** □ OAuth（提供商+回调 path+换会话后落点再勾 Cookie 或 Bearer）。**凡非「无登录」**：须钉会话探测端点（例 `GET /me`）与 401 语义 |
