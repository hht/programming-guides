...
4. 可见性超时：默认建议 **≥ 单次 Batch 墙钟**（INPUTS §11；默认起点 **300s**，长跑书面上调）。  
...
3. Airflow retry 与 `PIPELINE_MAX_ATTEMPTS` **取合同一上限策略**（书面：以谁为准）；禁无限重试。  
...
钉死默认映射（词表写入 `UBIQUITOUS_LANGUAGE.md`）：
