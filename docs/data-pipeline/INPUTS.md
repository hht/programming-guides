...
| # | 项 | 验收 |
|---|-----|------|
| 1 | **执行模式（互斥钉死一）** | □ **batch**（**默认**） □ **streaming**（须书面理由：持续事件源 + 延迟预算）— **禁止**「批或流任选」双开口；勾 streaming 时仍须遵守 `08` 四步映射，**不**另起核心路径名 |
| 2 | **Runner（互斥钉死一）** | □ **workers-queue**（**默认**：对齐 [workers-queue](../workers-queue/README.md)） □ **Airflow**（条件） □ **Dagster**（条件）— **禁止**默认钉 Airflow/Dagster；勾后两家须书面：≥N 依赖任务 / 跨仓调度 / 资产血缘产品需求 |
| 3 | **源清单** | ≥1 个 **Source**（类型：□ DB □ API □ 对象存储 □ 文件 □ 其它书面）；连接密钥名成对（值不入库）；抽取范围谓词（全量 / 增量水位字段） |
| 4 | **目标清单** | ≥1 个 **Destination**（□ PG 表 □ 对象前缀 □ 仓表 □ 其它书面）；装载语义钉死：□ append □ upsert（键列名）□ replace-partition（分区键） |
| 5 | **Dataset 契约** | 每个落地 Dataset：逻辑名、主键或自然键、字段表或 Schema URL、所有者团队；见 `03` |
| 6 | **幂等键策略** | 每次 **BatchRun** **必填** `idempotency_key`（业务词根 + 窗口/水位维度）；唯一性：□ 全局 □ 按 `pipeline_name`；冲突：□ **reject**（默认） □ resume-same-run（书面） |
...
| # | 项 | 何时 |
|---|-----|------|
| 11 | **workers-queue 对接** | Runner=workers-queue：**必填**队列名（例 `pipeline.orders_daily`）+ 可见性超时（默认对齐 workers-queue **300s** 或书面；长跑可更高） |
| 12 | **暂存介质** | 默认 □ **对象存储前缀**（对齐 object-storage）□ **PG staging schema** □ 内存仅单测；生产禁「只落 /tmp 无审计」 |
| 13 | **变换引擎** | □ **应用内变换**（默认）□ **dbt**（书面：SQL 模型仓路径）— 禁未钉死双引擎 |
...
| 17 | **流式等价** | 勾 streaming：钉死消息系统（□ Kafka □ Pub/Sub □ 其它书面）+ 消费组 + **exactly 映射** `ingest→extract` / `process→transform` / `sink→load` / `verify→verify` |
