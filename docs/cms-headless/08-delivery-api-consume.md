...
| **客户端直调** | 仅 Delivery **公开可读** token；仍禁 preview；CORS/scope 书面；错误映射同 BFF |
...
4. CMS 5xx → `CMS_UNAVAILABLE`；可回落 stale-if-error（INPUTS 钉：默认 **允许 stale ≤ TTL** 或 **硬失败** 二选一）。  
5. 列表/分页：若产品需要，单独钉 page size 默认 **20**、上限 **100**。
...
失效：publish/unpublish/webhook → delete 该键；禁「清全站」作唯一策略除非类型极少且书面。
