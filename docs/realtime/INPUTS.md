...
| 2 | **传输钉死** | **默认 WebSocket**。若勾 SSE：须书面理由（只读、穿透代理、无双向）+ 与 WS 能力边界表。**禁止**「WS 或 SSE 任选」双默认 |
| 3 | **应用册服务端** | □ **go**（默认库 **`github.com/coder/websocket`**）□ fastapi □ node/ts □ 多册（列清单）— 步骤按 `05` 语言无关；**禁止**同册第二默认 WS 库开口 |
| 4 | **WS 入口 URL 名** | staging/prod **成对**；变量名钉死（默认 `REALTIME_WS_URL`）；**值不入库** |
...
| 6 | **频道命名表** | 业务频道模式钉死（例 `order:{order_id}`、`user:{subject}:inbox`）；禁空频道、禁 `*` 裸通配进生产 ACL |
| 7 | **Authz 策略** | **互斥钉死一**：□ **每 subscribe 校验**（**默认**；连接鉴权不够）□ **连接时签发频道 claim 列表**（须书面：claim TTL、扩频道须重连）— **禁止**「连上就能订任意频道」 |
| 8 | **会话/令牌来源** | 对齐 [auth](../auth/README.md) 或产品 JWT：Cookie / `Authorization` / 首帧 `auth` 消息 — **互斥钉死一**；值不入库 |
...
| 10 | **心跳数字** | 默认 **ping 间隔 25s**、**超时 60s**（未收 pong/应用级心跳则断开）；改写须书面 |
...
| # | 项 | 何时 |
|---|-----|------|
| 14 | **SSE 路径** | 勾了 SSE：只读；事件名/id/`Last-Event-ID` 策略钉死；取消订阅 = **关流/换流**（与 11§B 共有行等价，**禁** N/A）；**不得**用 SSE 做默认双向命令通道 |
| 15 | **Ack 策略** | **互斥钉死一**：□ **至少一次 + 客户端去重（event_id）**（**默认**）□ **至多一次（服务端持久游标）**（须书面存储） |
| 16 | **背压** | 每连接出站队列上限默认 **256** 条或 **1 MiB**（先触达者触发）；超限行为钉死：□ 断开并 `BACKPRESSURE`（**默认**）□ 丢弃最旧并记指标（须书面） |
| 17 | **Redis 扇出** | 勾了多实例：对齐 [redis](../redis/README.md) INPUTS（`REDIS_URL`、前缀、TTL 若用键）；频道 → Redis channel 映射表；**单实例可 N/A** |
| 18 | **Socket.IO 映射** | 仅当遗留对接：书面「学边界不钉默认」；新仓 **禁止** 以 Socket.IO 为默认传输 |
