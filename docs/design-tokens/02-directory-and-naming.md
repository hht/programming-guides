...
2. **语义色**用用途名：`color.bg`、`color.fg`、`color.primary`、`color.danger`、`color.muted`（或 INPUTS 约定的等价集合）——**不是**设计师口语 `blue-2`、`coral-bright` 当唯一消费名。 
...
| 主题选择器 | light=`:root`；dark=INPUTS 约定（默认 `[data-theme="dark"]` 或 `.dark`）——**同一语义变量名** |
