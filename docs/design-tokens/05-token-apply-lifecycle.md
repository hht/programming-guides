...
- 顺序固定：**source tokens → transform → consume in UI**。 
...
2. 执行 `tokens:build`（名可约定）：读 DTCG → 平台 format → 写出产物（默认 `css/variables`）。 
...
| 需要「一次性」hex hotfix | 禁止进主路径；须补源 + rebuild（快改须写明 deferred） |
