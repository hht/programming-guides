...
| # | 项 | 验收（合格谓词） |
|---|-----|------------------|
| 1 | **源格式（互斥任选）** | □ **W3C DTCG**（`$value` / `$type`；本指南默认且唯一允许）。**禁止**「DTCG 或 legacy Style Dictionary 旧格式任选」双开口；存量迁移须有一次性计划，完成后只留 DTCG |
| 2 | **变换工具链（互斥任选）** | □ **Style Dictionary**（当前主线；DTCG 源）。**禁止**平行第二 transform（Theo / 自研 dump / 手写复制 CSS）作正式路径 |
| 3 | **色名 SSOT** | 语义色名表已约定（可复制 [templates/color-semantic-names.md](./templates/color-semantic-names.md)）；**禁止**第二套平行色名（如 `ink-*`、`faint`、`on-dark` 与 `foreground` / `muted-foreground` 并存） |
| 4 | **Token 分层** | 至少：`primitive` → `semantic`；组件层 □启用 □禁用。组件层若启用，只引用 semantic，禁直绑 primitive hex |
| 5 | **输出平台（≥1）** | 勾选且约定产物路径：□ **css-variables**（默认 Web）□ **js/ts module** □ **ios**（Swift / asset catalog 语义）□ **android**（XML / Compose theme）□ **其它**（书面 format 名） |
...
| 9 | **间距 / 字阶 / 圆角基线** | 约定：间距步进（默认 4）、正文字号/行高（Web 默认 16/24）、圆角档位数；与 ui-ux `01` 默认可对齐 |
...
| 12 | **环境 / CI** | PR 必跑 `tokens:build`（或约定名）+ drift check；产物 □提交仓内 □仅 CI 生成（择一；默认 **提交生成物或 lock 哈希** 以便 diff） |

## 若适用

| # | 项 | 何时 |
|---|-----|------|
| 13 | **Figma Variables 同步** | 有正式 Figma：约定同步方向（默认 **设计 Variables → 导出 DTCG → transform**；反向须写明） |
