...
**Request Lifecycle**：`接入 → 中间件注入 ctx → 解码+校验 → 应用用例 → 持久化(事务) → 编码响应 → 正确 status`。
...
| [06-persistence.md](./06-persistence.md) | sqlc / pgx / migrate |
...
```text
INPUTS → go mod + chi → 路由/中间件 → Request Lifecycle → sqlc → check → 验收
```
