...
2. **Introspection**：`APP_ENV=development` 可开；staging/prod 关闭 Yoga/插件 introspection，或仅运维身份/内网（INPUTS 书面）。探针：匿名 introspection 在 prod 配置下失败。  
3. **深度/复杂度**：默认建议 max depth **10**、复杂度预算产品钉死或 INPUTS §17；超限不执行。  
