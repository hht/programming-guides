...
- Schema 演进 = 契约变更：破坏性改名/删字段须版本策略或弃用期（INPUTS 书面）。  
...
3. Yoga（或书面例外的 Apollo）**从 SDL load** schema（`createSchema` / 等价）；禁默认 Pothos builder 当 SSOT。  
...
| 破坏性删字段未弃用 | 禁止合并；须弃用期或 INPUTS 书面 breaking |
