...
- CI 无 DB 时：`cargo sqlx prepare` 产物进 **`.sqlx/`** 并提交（或文档钉「CI 起 Postgres」二选一；**默认提交 `.sqlx/`**）

## 步骤规格

1. `migrations/0001_init.sql`（或 `.up.sql`/`.down.sql` 对——仓内钉一种；**默认 sqlx 可逆单文件版本链**）；CI/启动 `sqlx migrate run`。  
...
> `async_trait` 可选；Rust ≥1.75+ 可用 RPITIT / 手写 async fn in trait（edition 与 MSRV 一致时钉一种）。
