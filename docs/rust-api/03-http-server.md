...
> 说明：Go 册拆 ReadHeader/Read/Write/Idle；本栈以 **TimeoutLayer + body limit + graceful shutdown** 写明等价预算。若换 hyper 低层 builder，须在单处常量映射同等语义。

## 步骤规格

1. `main`：load config → `sqlx::PgPool` → migrate up（或独立 job；产品选定一种，**默认启动时 migrate up**）→ `server::router` → `TcpListener::bind` → serve + graceful shutdown。 
2. 中间件顺序：`RequestId` → **CatchPanic / 错误映射（panic→INTERNAL JSON，见 05）** → `tracing` 访问日志（tower-http TraceLayer 或自写）；**Auth 只挂在受保护 `Router` nest/merge**（见 `05`/`07`）。 
