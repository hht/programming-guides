...
1. **RequestID 中间件**：读 `X-Request-ID`；空则生成 UUIDv4（`uuid::Uuid::new_v4`）；写入 **请求 Extension**（类型钉死：`struct RequestId(pub String)`）+ 响应头 + tracing 字段 `request_id`（`Span::record` 或 `tracing::info_span!`）。  
2. 中间件顺序（钉死）：`RequestId` → **CatchPanic**（panic → `tracing::error!` + **`INTERNAL`** JSON，**禁止**默认明文 500）→ `TraceLayer` / 访问日志 → **Auth（仅受保护 nest）** → handler。  
3. **Handler 内顺序（钉死）**：  
