...
| # | 项 | 验收 |
|---|-----|------|
| 1 | **身份** | crate 名、服务名、公开 Base URL（staging/prod） |
...
| 6 | **鉴权** | **四选一**（互斥）：□ 无鉴权（仅内网） □ Bearer JWT（issuer/audience；HMAC→`JWT_SECRET`+默认 alg **HS256**，或 RS256+`JWT_PUBLIC_KEY_PEM`；**`sub`→`Subject` Extension**） □ Session cookie（cookie 名；**HttpOnly=true**；**Secure**=`APP_ENV!=development`；**Path=/**；**Max-Age** 对齐 `expires_at`；**SameSite 默认 Lax**；表默认 `sessions(id PK text, subject text NOT NULL, expires_at timestamptz NOT NULL, csrf_token text NOT NULL)`；CSRF 见 `07`） □ 外部 IdP（须写明 JWKS **或** introspection 二选一 + issuer + 凭证；**`sub`→`Subject` Extension**） |
