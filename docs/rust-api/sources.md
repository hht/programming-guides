...
| B | [loco-rs/loco](https://github.com/loco-rs/loco) | Axum 系生产向 Web/API 框架 | 配置、迁移习惯、健康检查、项目骨架 | 绑死 Loco 宏/代码生成全家桶 |
...
| 冲突 | 裁决 |
|------|------|
| Actix 下载/历史案例更多，或「Actix 或 Axum 任选」 | **钉 axum**（Tokio 一等、tower 生态、与 P0 examples 同栈）；**禁止**任选开口 |
| Diesel / SeaORM 更「ORM 全家桶」 | **钉 sqlx**（SQL 与类型显式；migrate 同库；先进优先） |
| warp / rocket 仍可见 | **钉 axum**（本指南品类默认；禁平行第二套） |
| `println!` / `log` 箱默认 | **钉 tracing**（async 友好；禁无理由再引第二套日志门面作默认） |
