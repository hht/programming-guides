...
| **配额** | 进程内令牌桶/信号量（单实例默认）或共享存储（多实例须 INPUTS 书面）；语义见 `06` |
| **超时** | `asyncio.wait_for` / runtime 原生 cancel + ASGI 层超时上限与 INPUTS 一致（取更严者） |

禁止：留下「FastAPI 或 Flask 任选」「ONNX 或随便一个」双开口；生产用未钉版本的 `latest`；指南内嵌可训练循环。
...
# 3) 实现 InferenceRuntime 适配器（仅 INPUTS 钉死的一家）；启动时 load + 校验 digest/semver
...
| 项 | 策略 |
|----|------|
| Python | 3.12+；lockfile（uv/poetry/pip-tools 三选一钉死在实现仓） |
| FastAPI / Pydantic | 跟当前稳定主版本；破坏性升级须改契约测试 |
| ONNX Runtime / 其它 runtime | 次版本钉死在 lockfile；与 `ARTIFACT_VERSION` 兼容矩阵书面一行 |
...
| 下载量第一的 serving 全家桶 | **不**单独定胜负；Lifecycle + 版本钉死 + 超时配额优先 |
