...
HTTP **模型推理服务边界**：授权 → 校验 → 推理 → 响应/超时；artifact 版本；超时与配额。**非**训练 / 微调 / 数据集百科。
...
|----|------|------|--------|----------|--------------|
| A | [bentoml/BentoML](https://github.com/bentoml/BentoML) | P1 | HTTP Service API、模型加载/管理、traffic timeout | 照搬 Bento 云；把整框架当唯一业务 SSOT；训练编排 | 把模型封成可 HTTP 调用的推理服务 |
| B | [vllm-project/vllm](https://github.com/vllm-project/vllm) | P1 | OpenAI 兼容 HTTP 推理、served model name、服务超时相关配置 | 将其当作仅 LLM 方案；把采样论文当本册正文；训练 | 高性能在线推理 HTTP 服务 |
| C | [triton-inference-server/server](https://github.com/triton-inference-server/server) | P1 | Model repository 版本、HTTP 推理协议、模型配置与超时相关项 | 将其当作 GPU 运维百科；把全部 backend 教程当应用分叉 | 生产级多框架推理服务器与版本化模型库 |
...
| 冲突 | 裁决 |
|------|------|
| BentoML/Triton 下载量/完整度 vs FastAPI 自写边界 | **默认 FastAPI + InferenceRuntime 端口**；标杆可学 timeout/版本/HTTP，不把它当作唯一实现框架 |
| vLLM 仅 LLM vs 通用推理品类 | vLLM 作 **HTTP 推理 + 模型名** 证据；通用路径默认 **ONNX Runtime**（INPUTS 可改 vLLM） |
| 生产用 `latest` 省事 | **禁止**作唯一解析；必须写明 semver 或 digest |
| 鉴权交给「内网」口头 | 须在 INPUTS §2 写明；默认仍要应用层 authorize |
