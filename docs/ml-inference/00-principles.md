...
2. **Artifact 版本**：进程（或可滚动单元）加载的 `model_id` + `artifact_version` 在启动/配置中显式；生产**禁止**仅靠浮动 `latest` 且无冻结 digest/semver。 
...
6. **单一 InferenceRuntime SSOT**：INPUTS 互斥任选一家实现；Lifecycle 与错误语义不随 runtime 分叉。 
...
- 跳过 authorize 的「内网就行」默认（须在 INPUTS 写明） 
- 无超时或无配额的生产验收路径 
