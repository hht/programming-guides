...
| # | 项 | 验收 |
|---|-----|------|
| 1 | **应用宿主（互斥任选一行）** | □ **Next.js App Router**（默认运行时 **`next-intl`**）□ **Vite SPA**（默认运行时 **`react-intl`**）□ **其它**（写明：库名 + 仍映射 Locale Resolve Lifecycle）— **禁止**「next-intl 或 i18next 任选」双开口 |
| 2 | **locale 列表** | BCP 47 标签 ≥1；标明 **默认 locale**（例 `en`）；支持列表与默认写入代码常量/配置 SSOT 一处 |
| 3 | **文案目录路径** | 例 `messages/{locale}.json` 或 `messages/{locale}/**/*.json`；格式采用 **JSON + ICU MessageFormat**；禁平行 YAML/TS 第二套生产源 |
| 4 | **检测序（主路径）** | 确认采用本册 `04` 默认序，或写明差分表（旧步→本册步）；须覆盖：显式用户选择、URL（若 Next）、持久化偏好、`Accept-Language`、默认 locale |
...
| 8 | **数字 / 日期时区** | 默认用 `Intl` + 当前 locale；默认时区策略：□ 用户偏好 □ 固定 `UTC` □ 固定 IANA 名 |
...
| # | 项 | 何时 |
|---|-----|------|
| 11 | **Next locale 路由** | 宿主=Next：□ `/{locale}/…` 前缀（**默认**）□ 域名分 locale — 互斥任选一种 |
| 12 | **消息分片** | 目录 > **INPUTS 约定的阈值**（默认建议 200 keys 或 50 KiB/文件）时：按 route/feature namespace 分片；加载策略见 `06` |
