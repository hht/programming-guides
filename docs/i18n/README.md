...
> **默认栈**：**文案 SSOT = JSON + ICU MessageFormat**；运行时按宿主互斥任选——**Next.js App Router → `next-intl`**；**Vite SPA（react 册）→ `react-intl`（FormatJS）**。学边界于 formatjs / i18next / next-intl（见 [sources.md](./sources.md)）；**禁止**把 i18next 定为默认；**禁止**组件内硬编码用户可见串作生产唯一源。 
...
1. [INPUTS.md](./INPUTS.md) → `INPUTS OK` 或停；宿主（Next / Vite SPA）与 locale 列表已互斥任选 
