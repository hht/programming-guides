...
- 顺序固定：**detect locale → load messages → render / missing-key fail**。 
...
3. 失败 → `i18n.messages_load_failed`；**停止**；UI = error / 重试，**禁止**用另一 locale 目录静默顶替除非 INPUTS 写明「仅缺文件时 fallback 默认 locale」且 **仍打日志**（默认：**不**跨 locale 静默顶替）。 
...
| 跳过 detect 并固定为 `en` | **禁止**当多语产品唯一策略（单语产品：支持列表仅默认，仍走 detect→load→render） |
