...
- 顺序固定（可 INPUTS 写明差分，但须仍覆盖各来源）；**不可**「每次渲染随机猜」。 
- 不在支持列表内的标签 → 映射到默认（或最大前缀匹配，INPUTS 约定；**默认 = 不匹配则 defaultLocale**）。
...
2. **持久化偏好**（cookie 名 INPUTS 约定，例 `NEXT_LOCALE`）：合法 → 采用，并可写入 URL 重定向策略（与 `next-intl` 一致，避免环）。 
3. **`Accept-Language`**：按 q 值选 **支持列表内** 最佳；无匹配 → 下一步。 
4. **默认 locale**（INPUTS §2）。

### Vite SPA（默认）

1. **显式用户选择**（设置页 / 语言切换写入的 storage 或 cookie；键名 INPUTS 约定）。 
...
1. 读取候选字符串（按序）。 
2. **规范化**：`zh-cn` → `zh-CN` 等与文件名一致的大小写规则（约定：跟 INPUTS 列表字面量匹配，大小写不敏感匹配后输出列表中的 canonical 标签）。 
...
| URL locale 不支持 | 重定向到默认或 404（INPUTS 约定；**默认重定向默认 locale**） |
