...
- 仅加载 **步骤 1 已解析** 的 `resolvedLocale`（及 INPUTS 书面允许的 fallback 文件策略）。  
...
| Vite SPA + `react-intl` | 静态 `import` 默认 locale；其它 locale **dynamic import()**（或全量打包若 locale≤3 且体积预算允许——INPUTS 钉） |
