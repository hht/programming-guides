...
3. **墙钟超时与 CPU 预算必须钉死数字或 INPUTS 必填项**；超时必须映射为调用方可感知失败（默认 `TIMEOUT` / 504），禁止无限挂起当成功。  
4. **写路径副作用必须幂等可重入**（HTTP 调用方重试或异步平台重试）；缺幂等维度的写 → 拒绝或 INPUTS 书面「只读/无副作用」。  
...
1. `对照：B 中冷启动/超时常作平台默认或文档散落 → 本指南要求墙钟超时与 CPU 预算钉死或进 INPUTS，且 TIMEOUT 必须映射为调用方可感知失败（见 06/07）`  
