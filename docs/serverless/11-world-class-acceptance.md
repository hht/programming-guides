...
> 仅 `sources` **共有必做**（**用户可感知**且 ≥2 源）。超时预算钉死、写路径幂等硬必填属 **超越 a***；本地/预览属 **工程面**（非共有，见 `sources` 差距表 / `03`/`09`）。实现仓仍须满足 `06`/`07` / §C / §D。
...
4. [ ] 工程面：§1.2 有章节  
5. [ ] 超越 a+b：  
   - [ ] a1. `对照：B 中冷启动/超时常作平台默认或文档散落 → 本指南要求墙钟超时与 CPU 预算钉死或进 INPUTS，且 TIMEOUT 必须映射为调用方可感知失败（见 06/07）`  
