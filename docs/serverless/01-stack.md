...
| OpenNext（若选） | 按 INPUTS §13 钉一目标适配器；App Router 对齐 [nextjs](../nextjs/README.md) |
...
禁止：留下「Workers 或 Vercel 任选」开口；默认钉 Lambda；边缘默认 Express/Koa。
...
cd <name>
pnpm add hono
# wrangler.toml：name、main、compatibility_date（钉 INPUTS）、vars 名（值按环境）
...
# 运行时 nodejs20.x；handler 文件路径钉死；事件源与超时在 IaC/控制台与 INPUTS 一致
```
...
| Wrangler | 跟 workers-sdk 稳定大版本；`compatibility_date` 钉死不漂移 |
...
| 冲突 | 裁决 |
|------|------|
| Vercel / Lambda 下载与心智份额更大 | **不钉为默认边缘**；OpenNext/Vercel、Lambda 仅 INPUTS；**默认 Cloudflare Workers**（先进优先） |
| Express 熟悉度 | **禁**作 Workers 默认；钉 Hono + Web Standards |
