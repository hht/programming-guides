...
| 2 | **相关 ID 名** | 对外字段名钉死（默认 **`correlation_id`**）；与 W3C `traceparent` 关系书面：□ 同值派生 □ 独立字段并存（**默认并存**：HTTP 用 W3C；应用日志字段保留 `correlation_id`） |
| 3 | **服务身份** | `service.name`（OTel Resource）钉死字符串；环境维 `deployment.environment` 名钉死（默认 `APP_ENV`） |
...
| 7 | **错误对外可追查** | 用户/调用方可感知策略钉死一：□ 响应头回显 `X-Correlation-Id`（**默认**）□ 响应 body 错误对象含 `correlation_id` □ 仅内部日志（须书面：支持场景如何取 id）— **禁止**「装了 SaaS 就算可追查」 |
...
| # | 项 | 何时 |
|---|-----|------|
| 11 | **采样** | 勾了 traces：默认 □ parentbased_always_on（dev）/ □ parentbased_traceidratio（prod，比率钉死，建议 `0.1` 起书面） |
| 12 | **前端 RUM** | Web 前端勾 traces/logs：□ 仅后端 □ 含浏览器（可映射 Faro/OTel JS）；须钉传播头是否跨 origin |
...
| 勾选 | 必读章 | 可 N/A |
|------|--------|--------|
| logs+traces（**默认**） | 03–06、08；（07 书面 N/A） | 07；INPUTS §6 |
