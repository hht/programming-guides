...
为服务/应用接入**日志、指标、追踪**（默认必做 logs + traces；metrics 可选）；同一请求可用 **correlation id** 关联查询/告警；规格写明，**不绑**商业 APM。
...
1. **标准写明**：**OpenTelemetry** 语义约定 + Resource（`service.name` 等）；日志默认 **结构化 JSON**。 
2. **导出将协议**：**OTLP**；后端实现可换（Jaeger / Grafana Tempo / 自建 collector 下游），**禁**把某家商业 APM SDK 标规定为默认必装。 
